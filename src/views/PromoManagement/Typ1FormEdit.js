...
export default function TableList(props) {
  ...
  const [iddisc, setIdDisc] = useState(props.iddisc);
  const [discname, setDiscname] = useState(props.discname);
  const [discstart, setDiscstart] = useState(props.discstart);
  const [discend, setDiscend] = useState(props.discend);
  const [disccontent, setDisccontent] = useState(props.disccontent);
  ...
  const postAxios = () => {
    ...
    axios({
      method: "put",
      url: "http://fd51fe99a1a1.ngrok.io/api/Discount",
      data: bodyFormData,
      headers: {
